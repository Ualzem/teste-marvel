function character() {
    ...
    if (queryParameterName !== null && queryParameterName !== "") {
        ...
        document.getElementById("characterSection").innerHTML =
            '<h2 id="characterMainTitle">Preencha o campo acima...</h2>';
    }
}
...
function connection() {
    ...
    xhr.onloadstart = function() {
        document.getElementById("characterSpinnerSection").innerHTML =
            '<strong id="spinnerText" class="text-primary">Buscando Personagem...</strong>' +
            '<div class="text-primary spinner-border ml-auto" role="status" ' +
            'aria-hidden="true" id="spinner"></div>';
    }
    xhr.onerror = function() {
        document.getElementById("characterSection").innerHTML = '<h2 id="characterMainTitle">Ocorreu um erro, confira sua conexão.</h2>';
    }
    xhr.onload = function() {
        if (this.status == 200) {
            ...
            if (results["data"].count === 0) {
                ...
                document.getElementById("characterSection").innerHTML =
                    ...
                    "Pode ter ocorrido um erro do nosso lado, desculpe. <br>Por favor, tente mais tarde.</h2>";
...
function comics(characterID) {
    ...
    xhr.onloadstart = function() {
        document.getElementById("comicsSpinnerSection").innerHTML =
            '<strong id="spinnerText" class="text-danger">Buscando revistas...</strong>' +
    ...
    xhr.onload = function() {
        if (this.status == 200) {
            ...
            if (results["data"] == 0) {
                ...
                comicSection.innerHTML = "<h2>Sem revistas avaliáveis</h2>";
            } else {
                // comics available
                let output = "",
                    creators = "";
    ...
    xhr.onloadend = function() {
        document.getElementById("comicsSpinnerSection").innerHTML =
            '<strong id="spinnerText" class="text-success">Pronto.</strong>';
    }
    xhr.send()
}
...
function singleComic() {
    ...
    xhr.open('GET', "./connections/single-revista.php?comic-id=" + comicID, true)
    xhr.setRequestHeader("X-Requested-With", "XMLHttpRequest");
    xhr.onloadstart = function() {
        document.getElementById("comicsSpinnerSection").innerHTML =
...
function comicCreator() {
    ...
    xhr.open("GET", "./connections/criador.php?creator-id=" + creatorID, true)
    xhr.setRequestHeader("X-Requested-With", "XMLHttpRequest");
    xhr.onloadstart = function() {
        document.getElementById("comicCreatorSpinnerSection").innerHTML =
            '<strong id="spinnerText" class="text-secondary">Buscando Info do Criador...</strong>' +
            '<div class="spinner-border text-secondary ml-auto" role="status" ' +
            'aria-hidden="true" id="spinner"></div>';
    }
    ...
    xhr.onload = function() {
        if (this.status == 200) {
            ...
            output +=
                '<h1 class="header-main-title single-comic__main-title">Revistas</h1>' +
            ...
            comicCreatorContainerDiv.innerHTML =
                '<h2 id="header-main-title single-comic__main-title">Ocorreu um erro, pode ser conexão ou requisição errada.</h2>';
        }
    }
...
function creatorSingleComic(comicResourceURI) {
    ...
    const xhr = new XMLHttpRequest();
    xhr.open("GET", "./connections/single-revista.php?comic-id=" + comicID)
    xhr.setRequestHeader("X-Requested-With", "XMLHttpRequest");
    xhr.onloadstart = function() {
        document.getElementById("comicCreatorSpinnerSection1").innerHTML =
            '<strong id="spinnerText" class="text-secondary">Buscando revistas...</strong>' +
            '<div class="spinner-border text-secondary ml-auto" role="status" ' +
            'aria-hidden="true" id="spinner"></div>';
    }
    xhr.onerror = function() {
